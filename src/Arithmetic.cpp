...
void Arithmetic::compress(std::string input_path, std::string output_path){
	...
	exec( ("python " + std::string(STRINGS_WORKING_DIR) + "lib/string_to_binary.py "+output_path).c_str());
}

void Arithmetic::uncompress(std::string input_path, std::string output_path){

	exec( ("python " + std::string(STRINGS_WORKING_DIR) + "lib/binary_to_string.py "+input_path).c_str());
